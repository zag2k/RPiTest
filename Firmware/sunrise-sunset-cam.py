...
sunrise_sunset = configParser.getboolean('camera', 'sunrise_sunset')
...
formatter = logging.Formatter('%(asctime)s - %(levelname)s - sunrise-sunset-cam.py - %(message)s')
...
if sunrise_sunset == True:
	if beg_nautical_twilight <= current_time <= beg_civil_twilight:
	...
	elif beg_civil_twilight <= current_time <= sunrise:
		res = cam.take(vflip=vflip, hflip=hflip, resolution_w=resolution_w, resolution_h=resolution_h, exposure_compensation=0, shutter_speed=1000000, exposure_mode="night", iso=100, file_name="/home/pi/" + application_folder + "/Daily/" + current_time.strftime("%Y-%m-%d-%H-%M-1") + ".jpg")
		res = cam.take(vflip=vflip, hflip=hflip, resolution_w=resolution_w, resolution_h=resolution_h, exposure_compensation=0, shutter_speed=0, exposure_mode="auto", iso=100, file_name="/home/pi/" + application_folder + "/Daily/" + current_time.strftime("%Y-%m-%d-%H-%M") + ".jpg")
	elif sunrise <= current_time <= sunset:
		res = cam.take(vflip=vflip, hflip=hflip, resolution_w=resolution_w, resolution_h=resolution_h, exposure_compensation=0, shutter_speed=shutter_speed, exposure_mode="beach", iso=100, file_name="/home/pi/" + application_folder + "/Daily/" + current_time.strftime("%Y-%m-%d-%H-%M") + ".jpg")
	elif sunset <= current_time <= end_civil_twilight:
		res = cam.take(vflip=vflip, hflip=hflip, resolution_w=resolution_w, resolution_h=resolution_h, exposure_compensation=0, shutter_speed=0, exposure_mode="auto", iso=100, file_name="/home/pi/" + application_folder + "/Daily/" + current_time.strftime("%Y-%m-%d-%H-%M") + ".jpg")
		res = cam.take(vflip=vflip, hflip=hflip, resolution_w=resolution_w, resolution_h=resolution_h, exposure_compensation=0, shutter_speed=1000000, exposure_mode="night", iso=100, file_name="/home/pi/" + application_folder + "/Daily/" + current_time.strftime("%Y-%m-%d-%H-%M-1") + ".jpg")
	elif end_civil_twilight <= current_time <= end_nautical_twilight:
		res = cam.take(vflip=vflip, hflip=hflip, resolution_w=resolution_w, resolution_h=resolution_h, exposure_compensation=0, shutter_speed=6000000, exposure_mode="verylong", iso=800, file_name="/home/pi/" + application_folder + "/Daily/" + current_time.strftime("%Y-%m-%d-%H-%M-6") + ".jpg")

	if res == True:
		logger.debug("Success!")
		upload.upload_files_in_folder("/home/pi/RPiWebCam/Daily/", "/" + camera_location + "/" + camera_name + "/Daily/" + date.today().strftime("%Y-%m-%d") + "/", file_description="Sunrise-Sunset Image", new_file_revision=False, delete_after_upload=True, notify=False)
		logger.debug("Upload done")
else:
	logger.info("Sunrise Sunset disabled in config.txt.")
